...
# Install Python and kaleido (run once)
tryCatch({
  # Try to use existing Python or install it
  if (!py_available()) {
    install_python()
  }
  # Install kaleido
...
# First, position 2007 businesses (sorted by type hierarchy)
...
# Then, position 2025 businesses (sorted by type hierarchy)
...
# Layout
fig <- fig %>% layout(
  title = "Business Transitions: Starting as Grocer or Raw Foods (2007-2025)",
  xaxis = list(
    title = "Year",
    tickmode = "linear",
    tick0 = start_year,
...
# Save as SVG
tryCatch({
  save_image(fig, "grocer_rawfoods_transitions.svg", width = 900, height = 600)
  cat("\nSVG saved as: grocer_rawfoods_transitions.svg\n")
}, error = function(e) {
  cat("\nCould not save SVG. Saving as HTML instead...\n")
  library(htmlwidgets)
  saveWidget(fig, "grocer_rawfoods_transitions.html", selfcontained = TRUE)
  cat("HTML saved as: grocer_rawfoods_transitions.html\n")
  cat("You can open this in a browser and use the camera icon to download as PNG/SVG\n")
})
