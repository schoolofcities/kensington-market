# Timeline visualization with scatter plot and S-curved lines - Ever Grocer or Raw foods
...
# Filter for businesses that are EVER a Grocer or Raw foods (at any point in their history)
...
for (feature in data$features) {
  ...
  # Check if ANY entry in the history is a Grocer or Raw foods
  has_grocer_or_raw <- FALSE
  for (entry in history) {
    if (!is.null(entry$year) && !is.null(entry$type)) {
      year <- entry$year
      if (year < start_year) year <- start_year
      if (year <= end_year) {
        if (entry$type %in% c("Grocer", "Raw foods")) {
          has_grocer_or_raw <- TRUE
          break
        }
      }
    }
  ...
  # Include this address if it ever becomes a Grocer or Raw foods
  if (has_grocer_or_raw) {
    included_addresses <- c(included_addresses, address)
  }
}

cat(sprintf("Found %d businesses that are ever Grocer or Raw foods\n", length(included_addresses)))
  ...
  for (entry in history) {
    if (!is.null(entry$year) && !is.null(entry$name) && !is.null(entry$type)) {
      ...
      if (year <= end_year) {
        ...
        # Create display_type: for Grocers, combine with subtype
        display_type <- if (entry$type == "Grocer" && subtype != "") {
          paste0("Grocer - ", subtype)
        } else {
          entry$type
        }
...
for (address in unique(all_businesses$address)) {
  ...
  if (last_entry$year < end_year) {
    all_businesses <- rbind(all_businesses, data.frame(
      ...
      subtype = last_entry$subtype,
      display_type = last_entry$display_type,
      address = address,
      stringsAsFactors = FALSE
    ))
  }
...
# For 2007 and 2025, count businesses by display_type and determine hierarchy
...
if (length(year_2007_idx) > 0) {
  ...
  year_data$type_rank <- match(year_data$display_type, year_type_hierarchy[[as.character(start_year)]])
  ...
  for (i in 1:nrow(year_data)) {
    ...
    if (nrow(business_history) > 1) {
      # Find the ultimate destination display_type (at 2025)
  ...
  # MANUAL OVERRIDE: Move Zimmerman's Fairland to bottom of ALL Grocer types (any subtype)
  zimm_idx <- which(grepl("Zimmerman.*Fairland", year_data$name, ignore.case = TRUE))
  if (length(zimm_idx) > 0) {
    # Find ALL Grocers in the sorted list (regardless of subtype or destination)
  ...
  if (nrow(business_history) > 1) {
    for (i in 1:(nrow(business_history) - 1)) {
      if (!is.na(business_history$y_pos[i]) && !is.na(business_history$y_pos[i + 1])) {
        ...
        source_display_type <- business_history$display_type[i]
        line_color <- if (!is.null(type_colors[[source_display_type]])) {
          # Add transparency to the color
          paste0(type_colors[[source_display_type]], "40")  # 40 is hex for ~25% opacity
        } else {
...
# Layout
fig <- fig %>% layout(
  title = "Business Transitions: Ever Grocer or Raw Foods (2007-2025) - OPTIMIZED FLOWS",
  xaxis = list(
    title = "Year",
    tickmode = "linear",
    tick0 = start_year,
...
fig <- fig %>% config(
  toImageButtonOptions = list(
    format = "svg",
    filename = "business_transitions_ever_grocer"
  )
)
...
saveWidget(fig, "business_transitions_ever_grocer.html", selfcontained = TRUE)